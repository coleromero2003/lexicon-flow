...
describe("Board Services", () => {
  ...
  afterEach(async () => {
    ...
    if (createdBoardId) {
      await supabase.from("boards").delete().eq("id", createdBoardId);
    }
  });
  ...
  describe("createBoard", () => {
    it("should create a new board", async () => {
      ...
      const result = await boardService.createBoard(supabase, newBoard);
      createdBoardId = result.id;

      expect(result).toBeDefined();
      expect(result.id).toBeTypeOf("number");
      expect(result.title).toBe(newBoard.title);
      expect(result.description).toBe(newBoard.description);
      expect(result.color).toBe(newBoard.color);
    ...
    it("should throw error when required fields are missing", async () => {
      ...
      await expect(
        boardService.createBoard(supabase, invalidBoard)
      ).rejects.toThrow();
    });
  });
  ...
  describe("getBoard", () => {
    beforeEach(async () => {
      ...
      const board = await boardService.createBoard(supabase, newBoard);
      createdBoardId = board.id;
    });

    it("should retrieve a board by ID", async () => {
      const board = await boardService.getBoard(supabase, createdBoardId);

      expect(board).toBeDefined();
      expect(board.id).toBe(createdBoardId);
      expect(board.title).toBe("Test Board");
    });

    it("should throw error when board does not exist", async () => {
      await expect(boardService.getBoard(supabase, 999999)).rejects.toThrow();
    });
  });
  ...
  describe("getBoards", () => {
    beforeEach(async () => {
      ...
      const created1 = await boardService.createBoard(supabase, board1);
      const created2 = await boardService.createBoard(supabase, board2);
      createdBoardId = created1.id; // Store for cleanup

      // Also cleanup second board
      afterEach(async () => {
        await supabase.from("boards").delete().eq("id", created2.id);
      });
    });

    it("should retrieve all boards for a user", async () => {
      const boards = await boardService.getBoards(supabase, testUserId);

      expect(boards).toBeDefined();
      expect(boards.length).toBeGreaterThanOrEqual(2);
      expect(boards.every((b) => b.user_id === testUserId)).toBe(true);
    });
  ...
  describe("updateBoard", () => {
    ...
    it("should update board properties", async () => {
      ...
      expect(updatedBoard.id).toBe(createdBoardId);
    });

    it("should update the updated_at timestamp", async () => {
      const originalBoard = await boardService.getBoard(
        supabase,
        createdBoardId
      );
      ...
      expect(new Date(updatedBoard.updated_at).getTime()).toBeGreaterThan(
        new Date(originalBoard.updated_at).getTime()
      );
    });
  });
});
...
describe("Column Services", () => {
  ...
  afterEach(async () => {
    // Cleanup
    if (createdColumnId) {
      await supabase.from("columns").delete().eq("id", createdColumnId);
    }
    if (testBoardId) {
  ...
  describe("createColumn", () => {
    it("should create a new column", async () => {
      ...
      const result = await columnService.createColumn(supabase, newColumn);
      createdColumnId = result.id;
  ...
  describe("getColumns", () => {
    beforeEach(async () => {
      const col1 = await columnService.createColumn(supabase, {
        board_id: testBoardId,
        title: "To Do",
        sort_order: 0,
        user_id: testUserId,
      });
      createdColumnId = col1.id;

      const col2 = await columnService.createColumn(supabase, {
        board_id: testBoardId,
        title: "In Progress",
        sort_order: 1,
        user_id: testUserId,
      ...
      afterEach(async () => {
        await supabase.from("columns").delete().eq("id", col2.id);
      });
    });

    it("should retrieve all columns for a board in sort order", async () => {
      const columns = await columnService.getColumns(supabase, testBoardId);

      expect(columns).toBeDefined();
      expect(columns.length).toBeGreaterThanOrEqual(2);
      expect(columns[0].sort_order).toBeLessThan(columns[1].sort_order);
  ...
  describe("updateColumnTitle", () => {
    ...
    it("should update column title", async () => {
      ...
      expect(updated.id).toBe(createdColumnId);
    });
  });
});
...
describe("Task Services", () => {
  ...
  describe("createTask", () => {
    it("should create a new task", async () => {
      ...
      const result = await taskService.createTask(supabase, newTask);
      ...
      expect(result.column_id).toBe(testColumnId);
    });
  });
  ...
  describe("getTasksByBoard", () => {
    ...
    it("should retrieve all tasks for a board", async () => {
      const tasks = await taskService.getTasksByBoard(supabase, testBoardId);
  ...
  describe("moveTask", () => {
    ...
    beforeEach(async () => {
      ...
      const column2 = await columnService.createColumn(supabase, {
        board_id: testBoardId,
        title: "In Progress",
        sort_order: 1,
        user_id: testUserId,
      });
      secondColumnId = column2.id;
    ...
    afterEach(async () => {
      if (secondColumnId) {
        await supabase.from("columns").delete().eq("id", secondColumnId);
      }
    });

    it("should move task to different column", async () => {
      await taskService.moveTask(supabase, createdTaskId, secondColumnId, 0);
      ...
      expect(movedTask?.column_id).toBe(secondColumnId);
      expect(movedTask?.sort_order).toBe(0);
    });
  });
});
